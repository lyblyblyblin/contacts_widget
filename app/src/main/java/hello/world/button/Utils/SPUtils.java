...
public class SPUtils {
     ...
     * 指定value寻找，返回key合集
     */
    public synchronized static List<String> findValue(Context context, Object object) {
        Map<String, ?> map;
        map = getAll(context);
        if (map.containsValue(object)) {
            List<String> list = new ArrayList<String>();
            for (int i = 0, n = 0; i < map.values().size(); i++) {
                if (map.values().toArray()[i].equals(object)) {
                    list.add(n,map.keySet().toArray()[i].toString());
                    n++;
                }
            }
            return list;
     ...
     * @return
     */
    public synchronized static Map<String, ?> getAll(Context context) {
        SharedPreferences sp = context.getSharedPreferences(FILE_NAME,
                Context.MODE_PRIVATE);
        return sp.getAll();
    }
    ...
    private static class SharedPreferencesCompat {
        ...
        private synchronized static Method findApplyMethod() {
            try {
